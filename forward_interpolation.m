...
close all;
clear all;
x = input('Enter x values separated with space with [ ] around them');
y = input('Enter f(x) values separated with space with [ ] around them');
...
%Formula: 
%f(x0)=y(1)+u*(del)y(1)+u*(u-1)/2!(del)^2y(1)+....+u*(u-1)..(u-n+1)/n!(del)^ny(1)
%where h=x(2)-x(1)=step length and u=(x0-x(1))/h. 
